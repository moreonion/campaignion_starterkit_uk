  ...
  public function exporter($type, $language) {
    ...
    switch ($type) {
      ...
      case 'csv':
        ...
        $map['mp_constituency']              = $labels->fromExporter(new WrapperField('mp_constituency'));
        $map['mp_party']                     = $labels->fromExporter(new TagField('mp_party'));
        $map['mp_salutation']                = $labels->fromExporter(new WrapperField('mp_salutation'));
